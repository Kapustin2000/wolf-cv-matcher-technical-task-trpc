...
export default class File {
    ...
    protected static uploadDir = path.dirname(fileURLToPath(import.meta.url));
    
    static async upload(file, fileName: string, uploadPath: string): Promise<Boolean> {

        const fullPath = path.join(this.uploadDir, uploadPath); // Use `this` to reference the static variable
